...
if (isset($_POST['name']) && isset($_POST['nazwisko']) 
    ...
    if(empty($name)){
        header("Location: admin_panel_admins.php?error=Imię jest wymagane&$user_data");
        exit();
    }elseif(empty($pass)){
        header("Location: admin_panel_admins.php?error=Hasło jest wymagane&$user_data");
        exit();
    }elseif(empty($nazwisko)){
        header("Location: admin_panel_admins.php?error=Nazwisko jest wymagane&$user_data");
        exit();
    }elseif(empty($uname)){
        header("Location: admin_panel_admins.php?error=E-mail jest wymagany&$user_data");
        exit();
        ...
        if (mysqli_num_rows($result) > 0){
            ...
            if($result2){
                ...
                header("Location: admin_panel_admins.php?error=Wystąpił nieznany błąd&$user_data");
                exit();
            }
        }
    }
